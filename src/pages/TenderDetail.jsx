  ...
  const submit = async () => {
    ...
    try {
      ...
      const ref = await createDoc('tenders', {
        ...
        bidSubmissionStart: ts(form.bidSubmissionStart),
        submissionDeadline: ts(form.submissionDeadline),
        preBidMeetingDate: ts(form.preBidMeetingDate),
        physicalSubmissionStart: ts(form.physicalSubmissionStart),
        physicalSubmissionEnd: ts(form.physicalSubmissionEnd),
        onlineOpeningDate: ts(form.onlineOpeningDate),
        resultDate: ts(form.resultDate),
        tenderFee: Number(form.tenderFee) || 0,
        emdAmount: Number(form.emdAmount) || 0,
        emdValidityDays: Number(form.emdValidityDays) || 0,
        pbgAmount: Number(form.pbgAmount) || 0,
        sdPercent: Number(form.sdPercent) || 5,
        estimatedValue: Number(form.estimatedValue) || 0,
        portalUrl: form.portalUrl,
        contactPerson: form.contactPerson,
        contactPhone: form.contactPhone,
        warrantyYears: Number(form.warrantyYears) || null,
        cmcYears: Number(form.cmcYears) || null,
      ...
      <Card>
        <CardContent>
          <Grid container spacing={2}>
            ...
            <Grid item xs={6} sm={4}><TextField fullWidth label="Batch / RC ID" placeholder="e.g. RC 2026-27" value={form.batchId} onChange={(e) => set('batchId', e.target.value)} /></Grid>
            <Grid item xs={6} sm={4}><TextField fullWidth type="number" label="Quantity" value={form.quantity} onChange={(e) => set('quantity', e.target.value)} /></Grid>

            <Grid item xs={12} sx={{ mt: 1 }}><Typography variant="subtitle2" color="primary">🌐 Portal & Contact</Typography></Grid>
            <Grid item xs={12} sm={6}>
              <TextField fullWidth select label="Common Portal" value=""
                onChange={(e) => { const found = TenderPortals.find(p => p.name === e.target.value); if (found) set('portalUrl', found.url) }}>
                <MenuItem value="">-- Select to fill URL --</MenuItem>
                {TenderPortals.map(p => <MenuItem key={p.name} value={p.name}>{p.name}</MenuItem>)}
              </TextField>
            </Grid>
            <Grid item xs={12} sm={6}><TextField fullWidth label="Portal URL" value={form.portalUrl} onChange={(e) => set('portalUrl', e.target.value)} /></Grid>
            <Grid item xs={12} sm={6}><TextField fullWidth label="Contact Person" value={form.contactPerson} onChange={(e) => set('contactPerson', e.target.value)} /></Grid>
            <Grid item xs={12} sm={6}><TextField fullWidth label="Contact Phone" value={form.contactPhone} onChange={(e) => set('contactPhone', e.target.value)} /></Grid>

            <Grid item xs={12} sx={{ mt: 1 }}><Typography variant="subtitle2" color="primary">📅 Timeline</Typography></Grid>
            <Grid item xs={12} sm={6}><TextField fullWidth type="datetime-local" label="Bid Submission Start" InputLabelProps={{ shrink: true }} value={form.bidSubmissionStart} onChange={(e) => set('bidSubmissionStart', e.target.value)} /></Grid>
            ...
            <Grid item xs={12} sm={6}><TextField fullWidth type="datetime-local" label="Pre-bid Meeting" InputLabelProps={{ shrink: true }} value={form.preBidMeetingDate} onChange={(e) => set('preBidMeetingDate', e.target.value)} /></Grid>
            <Grid item xs={12} sm={6}><TextField fullWidth type="datetime-local" label="Online Opening" InputLabelProps={{ shrink: true }} value={form.onlineOpeningDate} onChange={(e) => set('onlineOpeningDate', e.target.value)} /></Grid>
            <Grid item xs={12} sm={6}><TextField fullWidth type="date" label="Physical Submission Start" InputLabelProps={{ shrink: true }} value={form.physicalSubmissionStart} onChange={(e) => set('physicalSubmissionStart', e.target.value)} /></Grid>
            <Grid item xs={12} sm={6}><TextField fullWidth type="date" label="Physical Submission End" InputLabelProps={{ shrink: true }} value={form.physicalSubmissionEnd} onChange={(e) => set('physicalSubmissionEnd', e.target.value)} /></Grid>
            ...
            <Grid item xs={12} sx={{ mt: 1 }}><Typography variant="subtitle2" color="primary">💰 Money & Guarantees</Typography></Grid>
            ...
            <Grid item xs={6} sm={3}><TextField fullWidth type="number" label="Tender Fee (₹)" value={form.tenderFee} onChange={(e) => set('tenderFee', e.target.value)} /></Grid>
            ...
            <Grid item xs={6} sm={3}><TextField fullWidth type="number" label="SD %" value={form.sdPercent} onChange={(e) => set('sdPercent', e.target.value)} /></Grid>

            <Grid item xs={12} sx={{ mt: 1 }}><Typography variant="subtitle2" color="primary">🛠️ Contract Terms</Typography></Grid>
            <Grid item xs={6} sm={4}>
              <TextField fullWidth select label="Warranty (years)" value={form.warrantyYears} onChange={(e) => set('warrantyYears', e.target.value)}>
                <MenuItem value="">-- Not set --</MenuItem>
                {WARRANTY_OPTIONS.map(y => <MenuItem key={y} value={y}>{y} years</MenuItem>)}
              </TextField>
            </Grid>
            <Grid item xs={6} sm={4}>
              <TextField fullWidth select label="CMC (years)" value={form.cmcYears} onChange={(e) => set('cmcYears', e.target.value)}>
                <MenuItem value="">-- Not set --</MenuItem>
                {CMC_OPTIONS.map(y => <MenuItem key={y} value={y}>{y === 0 ? 'No CMC' : `${y} years`}</MenuItem>)}
              </TextField>
            </Grid>
            <Grid item xs={6} sm={4}><TextField fullWidth type="number" label="Delivery Days" value={form.deliveryDays} onChange={(e) => set('deliveryDays', e.target.value)} /></Grid>
